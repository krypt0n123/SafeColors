...
function GroupNode({ id, data }: NodeProps<Node<NodeData>>) {
    ...
    return (
      <div className="flex flex-wrap gap-2">
        {groups.map((group, index) => (
          <div
            key={index}
            className="flex flex-col px-3 py-2 bg-gray-800 text-white rounded-md text-sm font-mono"
          >
            {group}
          </div>
        ))}
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-md border border-gray-200">
      ...
      />
      <div className="text-sm font-medium text-gray-700 mb-2">Hash Groups:</div>
